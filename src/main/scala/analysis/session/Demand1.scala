...
object Demand1 {
  def main(args: Array[String]): Unit = {
    ...
    //任务执行的ID，用户唯一标示运行后的结果，用在MySQL数据库中
    val taskUUID = UUID.randomUUID().toString

    //从db_userbehaviors.user_visit_action读出数据,查询指定日期范围内的行为数据
    val actionRDD = Demand1Function.getActionRDDByDateRange(taskParam) //UserVisitAction(2020-01-07,91,1235481e78ec49f986f00651709450b2,8,2020-01-07 1:39:32,太古商场,-1,-1,null,null,null,null,4)
    ...
    //将数据进行缓存处理
//    sc.setCheckpointDir("hdfs://bd1:9000/AnalysisUserBehaviors/SessionId2ActionRDD")
...
//    //将数据进行缓存处理
//    sc.setCheckpointDir("hdfs://bd1:9000/AnalysisUserBehaviors/session_id2AggrInfoRDD")
//    session_id2AggrInfoRDD.checkpoint()
